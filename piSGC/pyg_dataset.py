...
from utils import load_pyg_dataset, sgc_precompute, set_seed
from models import get_model
from metrics import accuracy
import pickle as pkl
from args import get_pyg_args
...
print(data_path)
print(args.dataset)
print("model degree: {}".format(args.degree))
print("alpha: {}".format(args.alpha))
print("aggr type: {}".format(args.aggr_type))
print("norm type: {}".format(args.norm_type))

adj, features, labels, idx_train, idx_val, idx_test = load_pyg_dataset(args.dataset, args, args.normalization, 0,
                                                                       args.cuda, data_path)

if args.dataset in {'cornell', 'film', 'texas', 'wisconsin'}:
    # test
    data_path = './pinv-dataset/ectd_' + args.dataset + '_'
    if args.using_vg: data_path += 'vg' + str(args.vg) + '_'
    data_path += args.method + '_' + args.adj + '.txt'
    ...
    adj, features, labels, idx_train, idx_val, idx_test = load_webkb(args.dataset, splits,
                                                                     None, None, args.normalization, args.cuda,
                                                                     data_path,
                                                                     args)
...
if args.model == "SGC" or args.model == "MSGC":
    features, precompute_time = sgc_precompute(features, adj, args.degree, args)
